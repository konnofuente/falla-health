  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        ...
        const sortedHospitals = hospitalList
          .map((hospital) => {
    ...
    if (navigator.share) {
      navigator
        .share({
          ...
          text: `Hello, j'espère que tu vas bien\n\n ! Je viens de découvrir que l'hôpital ${hospital.name} propose des dépistages du sein contre le cancer, et je voulais partager ca avec toi.\n Je l'ai trouvé sur le site FallaCare de CamaireTech: https://camairetech.com/operation-octobre-rose/ Visite le site pour trouver un autre hôpital plus proche de chez toi !`,
          url: shareUrl,
        })
      ...
      navigator.clipboard
        .writeText(shareUrl)
        .then(() =>
          alert("Le lien de l'hôpital a été copié dans le presse-papiers !")
        )
            ...
            <div className="hospital-list">
              ...
              <ul>
                {hospitals.map((hospital) => (
                  <li key={hospital.id} className="hospital-item">
                      ...
                      <p>
                        <span className="open">
                          {hospital.ouvertureDate
                            ? `Du ${hospital.ouvertureDate} au ${
                                hospital.fermetureDate ||
                                "date de fermeture inconnue"
                              }`
                            : "Période d'ouverture non disponible"}
                        </span>{" "}
                        <br></br>
                        {hospital.ouvertureTime && (
                          <span className="close">
                            <span className="open-label">Heure :</span>{" "}
                            {`De ${hospital.ouvertureTime} à ${
                              hospital.fermetureTime ||
                    ...
                    {hospital.price ? (
                      <p className="hospital-price">
                        Prix: {hospital.reductionPrice ? <s>{hospital.price} FCFA</s> : `${hospital.price} FCFA`}
                      ...
                      <p className="hospital-price">
                        Veuillez visiter ou appeler pour connaître les prix.
                      </p>
                    )}
                  ...
                  >
                    {hospital.phone ? "Appeler" : "Numéro non disponible"}
                  </button>
